...
import { 
  ...
  Code, FileJson, Copy, HeartPulse, HardDrive, Beaker, FileSearch, Edit3
} from 'lucide-react';
...
const CustomerCenterPage: React.FC = () => {
  ...
  const [editBuffer, setEditBuffer] = useState<{vrm?: string, make?: string, model?: string, type?: string}>({});
                    ...
                    <div className="bg-gray-50 p-8 rounded-[40px] border border-gray-100">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                             <p className="text-[9px] font-black uppercase text-pink-500 mb-2">Comp. (Car/Van)</p>
                             <p className="text-lg font-black font-outfit text-[#2d1f2d]">£3,000 – £3,999</p>
                          </div>
                          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                             <p className="text-[9px] font-black uppercase text-blue-500 mb-2">T.P.F.T (Car/Van)</p>
                             <p className="text-lg font-black font-outfit text-[#2d1f2d]">£1,400 – £2,999</p>
                          </div>
                          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                             <p className="text-[9px] font-black uppercase text-indigo-500 mb-2">Motorcycle</p>
                            ...
                            return (
                              <tr key={p.id} className="bg-gray-50 group hover:bg-white hover:shadow-xl transition-all rounded-[32px]">
                                <td className="px-6 py-5 rounded-l-[24px]">
                                  <button onClick={() => { setActingPolicy(p); setEditBuffer({ vrm: p.details?.vrm, make: p.details?.make, model: p.details?.model, type: p.type }); }} className="flex flex-col text-left group-hover:translate-x-1 transition-transform">
                                ...
                                <td className="px-6 py-5">
                                  <div className="flex items-center gap-3">
                                    ...
                                    {p.pdfUrl ? (
                                      <CheckCircle2 size={14} className="text-[#e91e8c]" title="Certificate Generated" />
                                    ) : (
                                      <FileSearch size={14} className="text-gray-300" title="Certificate Pending" />
                                    )}
                                  </div>
                                </td>
                                <td className="px-6 py-5 rounded-r-[24px] text-right">
                                  <div className="flex justify-end gap-2">
                                    <button 
                                      onClick={() => { setActingPolicy(p); setEditBuffer({ vrm: p.details?.vrm, make: p.details?.make, model: p.details?.model, type: p.type }); }}
                                      className="p-2 bg-white text-gray-400 hover:text-indigo-500 rounded-lg transition-all"
                                      title="Edit Policy Manifest"
                                    >
                                      <Edit3 size={14} />
                                    </button>
                                    {p.pdfUrl && (
                                      <button onClick={() => downloadPDF(p.id)} className="p-2 bg-pink-50 text-[#e91e8c] rounded-lg hover:bg-[#e91e8c] hover:text-white transition-all" title="Verify PDF"><Download size={14}/></button>
                                    )}
                                    ...
                                    <button onClick={() => { if(window.confirm('IRREVERSIBLE: Execute permanent deletion for ' + p.id + '?')) deletePolicy(p.id, 'Admin Permanent Deletion'); }} className="px-3 py-1.5 bg-red-600 text-white text-[9px] font-black uppercase tracking-widest rounded-lg hover:bg-red-700 transition-all">Delete</button>
                                  </div>
                                </td>
                              </tr>
                            );
           ...
           <div className="bg-white w-full max-w-2xl rounded-[48px] p-12 relative shadow-2xl animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex items-center justify-between mb-10">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h2 className="text-3xl font-bold text-[#2d1f2d] font-outfit">Policy Editor</h2>
                    <p className="text-gray-400 text-sm font-medium tracking-tight">ID: {actingPolicy.id}</p>
                  </div>
                </div>
                <button 
              ...
              {showManifest ? (
                <div className="space-y-6 animate-in fade-in duration-300">
                   <div className="flex items-center justify-between">
                      <h3 className="text-xs font-black uppercase tracking-widest text-gray-400">Data Integrity Inspector</h3>
                      <span className="text-[10px] font-mono bg-indigo-50 text-indigo-600 px-2 py-1 rounded">JSON:VALID</span>
                   </div>
                   ...
                   <button onClick={() => setShowManifest(false)} className="w-full py-4 bg-gray-100 text-gray-400 rounded-xl font-black uppercase tracking-widest text-[10px]">Return to Editor</button>
                </div>
              ) : (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Vehicle Model</label>
                        <input className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 font-bold text-[#2d1f2d] outline-none focus:border-[#e91e8c]" value={editBuffer.model} onChange={e => setEditBuffer({...editBuffer, model: e.target.value})} />
                      </div>
                  </div>
                  ...
                  <div className="p-4 bg-orange-50 border border-orange-100 rounded-xl text-[10px] text-orange-600 font-bold flex gap-3">
                      <AlertTriangle size={14} className="shrink-0" /> Note: Updating policy manifest details will require a PDF certificate regeneration for the customer.
                  </div>
                </div>
              )}
           </div>
                       ...
                       {policies.filter(p => p.userId === actingUser.id).map(p => (
                         <div key={p.id} className="p-4 bg-gray-50 border border-gray-100 rounded-2xl flex items-center justify-between">
                            <div>
                               ...
                               <p className="text-xs font-bold text-[#2d1f2d]">{p.type}</p>
                            </div>
